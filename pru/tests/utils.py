...
def load(
    ...
    if avoid_cache:
        ...
        for path in include_paths:
            ...
            header_files.extend(glob.glob(path + "/*.h"))
        module = get_cached_module(module_name, source_files + header_files)
...
def get_cached_module(module_name: str, sources: list[str]):
    try:
        module = importlib.import_module(module_name)
    except ModuleNotFoundError:
        return None

    cache_time = os.path.getmtime(module.__file__)
    for source in sources:
        if os.path.getmtime(source) > cache_time:
            return None
