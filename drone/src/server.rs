...
use futures::{select, Stream, StreamExt, TryStreamExt, FutureExt};
use tokio::sync::{
    broadcast::{Sender, Receiver, RecvError},
    RwLock,
};
use warp::Filter;
use std::{
...
use message::{Readable, Writable, DrosixMessage};

#[cfg(not(feature = "mock"))]
const RTC_ADDR: &'static str = "192.168.7.2:3333";
#[cfg(feature = "mock")]
...
impl RtcStatus {
    ...
    pub fn add_client(&mut self, addr: SocketAddr) -> Option<u32> {
        if !self
            .clients
            .values()
            .any(|&client_addr| client_addr == addr)
        {
...
impl RtcServer {
    ...
    async fn run(mut self, mut receiver: Receiver<[f64; 3]>) {
    ...
    async fn connect(
        mut endpoint: webrtc::SessionEndpoint,
        body: impl Stream<Item = Result<impl bytes::Buf, warp::Error>> + Send + Sync,
    ) -> Result<impl warp::Reply, warp::Rejection> {
        match endpoint
            .http_session_request(body.map_ok(|mut buf| {
                buf.to_bytes()
            }))
        .await
        {
    ...
    async fn receive(&mut self, msg_status: webrtc::MessageResult, data: &[u8]) {
        match DrosixMessage::read_from_buffer(&data[..msg_status.message_len]) {
            Ok(DrosixMessage::ClientHello) => {
                ...
                if let Some(id) = status.add_client(msg_status.remote_addr) {
                    let msg = DrosixMessage::ServerHello(id).write_to_vec().unwrap();
                    self.server.send(
                        &msg,
    ...
    async fn send(&mut self, data: [f64;3]) {
        ...
        for client in status.measure_clients.iter() {
            let _ = self.server.send(&msg, webrtc::MessageType::Binary, client).await;
        }
    }
}

#[tokio::main]
pub async fn server(sender: Sender<[f64; 3]>) {
    let rtc_address = SocketAddr::from_str(RTC_ADDR).unwrap();
    let rtc_server = RtcServer::new(rtc_address).await.unwrap();
    ...
    let unsubscribe = warp::path!("measure" / u32)
        .and(rtc_status)
        .and(warp::delete())
        .and_then(RtcServer::unsubscribe);

    let api = warp::path("api").and(webrtc.or(subscribe).or(unsubscribe));

    let app = warp::fs::dir(FILES);

    let http_server = warp::serve(app.or(api)).run(HTTP_ADDR);
    let rtc_server = rtc_server.run(sender.subscribe());
